...
# ✅ INPUTS
# =========================
REPORT_FILTER_VALUE = "District Wise Installations ASC"  # <-- set any option text from the first dropdown
STATE_NAME = "GUJARAT"
...
# Load more
LOAD_MORE_MAX_CLICKS = 600
LOAD_MORE_STUCK_LIMIT = 5  # ✅ requirement: stop after 5 no-growth tries
LOAD_MORE_POST_CLICK_WAIT_MS = 25_000
...
PDROPDOWN_TRIGGER = ".p-dropdown-trigger"             # ✅ requested
...
def error_toast_visible(page) -> bool:
    # matches the screenshot "Error 500" / "Unable to fetch details"
    try:
        return (
...
# -------------------------
# Filters panel scoping (prevents clicking Login)
# -------------------------
def get_filters_panel(page):
    """
    Returns the left filters panel container (scoped to Apply button).
    """
    filters_txt = page.get_by_text("Filters", exact=False).first
    filters_txt.wait_for(timeout=15_000)

    # Find nearest ancestor that contains the Apply button (safe scope)
...
# -------------------------
# PrimeNG Report dropdown selection (p-dropdown-trigger)
# -------------------------
def _norm(s: str) -> str:
...
def select_report_filter(panel, page, value_text: str):
    """
    Selects the first report dropdown ("District Wise Inst...") using PrimeNG p-dropdown.
    It searches within the Filters panel for a p-dropdown that contains "District Wise".
    """
    ...
    for attempt in range(1, MAX_REFRESH_RETRIES + 1):
        ...
        # Prefer: dropdown that currently contains "District Wise" (unique in your UI)
        dd = panel.locator(PDROPDOWN_ROOT).filter(
            has_text=re.compile(r"District\s*Wise", re.I)
        ).first

        # Fallback: first p-dropdown inside filters panel
        if dd.count() == 0:
            dd = panel.locator(PDROPDOWN_ROOT).first

        if dd.count() == 0:
            print(f"[WARN] No PrimeNG p-dropdown found in Filters panel. Refresh {attempt}/{MAX_REFRESH_RETRIES}")
            if not hard_refresh(page, URL, tries=2):
                raise RuntimeError("Hard refresh failed while locating report dropdown.")
            wait_spa_ready(page)
        ...
        panel_overlay = page.locator(PDROPDOWN_PANEL).filter(has_text=re.compile(r".*", re.S)).last
        try:
            panel_overlay.wait_for(state="visible", timeout=OPTIONS_WAIT_MS)
        except Exception:
            print(f"[WARN] p-dropdown panel did not open. Refresh {attempt}/{MAX_REFRESH_RETRIES}")
            continue

        # If filter input exists, use it
        try:
            filt = panel_overlay.locator(PDROPDOWN_FILTER_INPUT).first
        ...
        wrapper = panel_overlay.locator(PDROPDOWN_ITEMS_WRAPPER).first

        clicked = False
        for _ in range(80):
            try:
                clicked = panel_overlay.evaluate(
                    """
                    (el, wanted) => {
                      const norm = (s) => (s||"")
                        .replace(/…|\\.{3}/g,"")
                      ...
                      const items = Array.from(el.querySelectorAll(".p-dropdown-item"));

                      // exact match
                      for (const it of items) {
                        if (norm(it.textContent) === wanted) { it.click(); return true; }
                      }
                      // contains match fallback
                      for (const it of items) {
                        if (norm(it.textContent).includes(wanted)) { it.click(); return true; }
                      }
                      return false;
                    }
                    """,
            ...
            try:
                if wrapper.count() > 0:
                    wrapper.evaluate("el => { el.scrollTop = el.scrollTop + el.clientHeight * 0.8; }")
            except Exception:
                pass
        ...
        if not clicked:
            close_any_dropdown(page)
            print(f"[WARN] Could not click report option '{wanted_raw}'. Retry {attempt}/{MAX_REFRESH_RETRIES}")
            continue
        ...
        try:
            label_now = (dd.locator(PDROPDOWN_LABEL).first.inner_text() or "").strip()
        except Exception:
            label_now = ""

        if wanted in _norm(label_now):
            print(f"[OK] Report filter selected: {label_now}")
            return

        # If truncated, still accept if it starts with same words
        if _norm(label_now) and _norm(label_now) in wanted:
            print(f"[OK] Report filter selected (truncated label): {label_now}")
            return

        print(f"[WARN] Report label not updated. Got='{label_now}'. Retry {attempt}/{MAX_REFRESH_RETRIES}")

    raise RuntimeError(f"Failed to select report filter '{value_text}' after retries.")
...
def click_placeholder_dropdown_in_panel(panel, placeholder_text: str):
    """
    IMPORTANT: scoped to Filters panel so it doesn't click Login/top navbar.
    """
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # ✅ 1) Select Report filter (PrimeNG p-dropdown)
            select_report_filter(panel, page, REPORT_FILTER_VALUE)

            # ✅ 2) Select State + District (ng-select placeholders inside Filters panel)
            ...
            # Error 500 toast -> retry once
            if error_toast_visible(page):
                print("[WARN] Error toast detected after Apply. Backoff 6s then refresh+re-apply once.")
